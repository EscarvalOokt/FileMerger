...
public sealed class MergeSessionValidator : IMergeSessionValidator
{
    ...
    private static void ValidateSession(MergeSession session, List<ValidationIssue> issues)
    {
        if (string.IsNullOrWhiteSpace(session.Name))
        {
            issues.Add(Error("session.name.empty", "Session name cannot be empty."));
        }
    }

    private static void ValidateSources(MergeSession session, List<ValidationIssue> issues)
    {
        if (session.Sources.Count == 0)
        {
            issues.Add(Error("session.sources.empty", "At least one source must be specified."));
            return;
        }
        ...
        {
            issues.Add(Error("session.sources.noneEnabled", "At least one enabled source must be specified."));
            return;
        }
        ...
        foreach (MergeSource source in enabledSources)
        {
            ...
            {
                issues.Add(Error("source.path.empty", "Source path cannot be empty."));
                continue;
            }
            ...
            {
                issues.Add(Error("source.path.invalid", $"Source path is invalid: '{source.Path}'."));
                continue;
            }

            if (!normalizedMap.TryAdd(normalizedPath, source))
            {
                issues.Add(Warning("source.path.duplicate", $"Duplicate source path detected: '{normalizedPath}'."));
            }
            ...
            switch (source.Type)
            {
                case MergeSourceType.Directory:
                    ...
                    {
                        issues.Add(
                            Error("source.directory.notFound", $"Source directory was not found: '{normalizedPath}'."));
                    }
                ...
                case MergeSourceType.File:
                    ...
                    {
                        issues.Add(Error("source.file.notFound", $"Source file was not found: '{normalizedPath}'."));
                    }
        ...
        for (int i = 0; i < normalizedSources.Length; i++)
        {
            for (int j = i + 1; j < normalizedSources.Length; j++)
            {
                ...
                if (left.Type != MergeSourceType.Directory && right.Type != MergeSourceType.Directory)
                {
                    continue;
                }
    ...
    private static void ValidateProfile(MergeSession session, List<ValidationIssue> issues)
    {
        if (session.Profile.FileTypes.Count == 0 || !session.Profile.FileTypes.Any(x => x.IsEnabled))
        {
            issues.Add(Error("profile.fileTypes.empty", "At least one enabled file type must be selected."));
        }
    ...
    private static void ValidateOutput(MergeSession session, List<ValidationIssue> issues)
    {
        if (string.IsNullOrWhiteSpace(session.OutputTarget.Path))
        {
            issues.Add(Error("output.path.empty", "Output path cannot be empty."));
            return;
        }
        ...
        {
            issues.Add(Error("output.path.invalid", $"Output path is invalid: '{session.OutputTarget.Path}'."));
            return;
        }

        if (string.IsNullOrWhiteSpace(session.OutputTarget.EncodingName))
        {
            issues.Add(Error("output.encoding.empty", "Output encoding cannot be empty."));
        }
        ...
        foreach (MergeSource source in session.Sources.Where(x => x.IsEnabled))
        {
            ...
            {
                issues.Add(
                    Warning("output.conflicts.withSourceFile", "Output path matches one of the enabled file sources."));
            }
