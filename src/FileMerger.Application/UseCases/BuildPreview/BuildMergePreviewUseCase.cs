...
public sealed class BuildMergePreviewUseCase
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return Task.Run(
            () =>
            {
                ...
                FileDiscoveryResult discoveryResult = _fileDiscoveryService.DiscoverFiles(
                ...
                try
                {
                    filteredFiles = _fileFilterService.ApplyFilters(mergeCandidates, validatedSession.Profile);
                }
                catch (RegexMatchTimeoutException)
                {
                    var timeoutIssue = new ValidationIssue(
                        severity: ValidationSeverity.Error,
                        code: "profile.filterRules.invalid",
                        message:
                        "Regular expression filter evaluation exceeded the 250 ms timeout. Simplify the pattern and try again.");

                    List<ValidationIssue> timeoutIssues = [.. validationIssues, timeoutIssue];
                    MergeSession failedSession = validatedSession.WithValidationIssues(timeoutIssues);

                    return new BuildMergePreviewResult(
                        session: failedSession,
                        output: null,
                        automaticFiles: [],
                        validationIssues: timeoutIssues,
                        isSuccessful: false,
                ...
                IReadOnlyCollection<InputFile> overriddenFiles =
                    _fileInclusionOverrideService.ApplyOverrides(filteredFiles, request.InclusionOverrides);

                IReadOnlyCollection<InputFile> effectiveFiles = ReconcileInventory(automaticFiles, overriddenFiles);
                ...
                MergeSession finalSession = processedSession.WithValidationIssues(allIssues).WithLastOutput(output);
    ...
    {
        var updatedMap = updatedFiles.GroupBy(x => x.FullPath, StringComparer.OrdinalIgnoreCase)
            .ToDictionary(x => x.Key, x => x.Last(), StringComparer.OrdinalIgnoreCase);

        return
        [
            .. baselineFiles.Select(file => updatedMap.GetValueOrDefault(file.FullPath, file))
        ];
    }

    private static string BuildSectionHeaderText(InputFile file, string? encodingName, MergeSession session)
    {
        ...
        return string.IsNullOrWhiteSpace(encodingName) ? file.RelativePath : $"{file.RelativePath} [{encodingName}]";
    }

    private sealed class DiscoveryProgressAdapter(IProgress<BuildMergePreviewProgress> progress)
        : IProgress<FileDiscoveryProgress>
    {
